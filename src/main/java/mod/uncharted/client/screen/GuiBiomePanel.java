...
import static mod.uncharted.Uncharted.MODID;
...
public class GuiBiomePanel { // extends GuiComponent {
	...
	public GuiBiomePanel(Minecraft mc) {
		...
		// smallFrame  = UnchartedConfig.CONFIG.smallFrame.get();
		// borderLower = UnchartedConfig.CONFIG.borderLower.get();
		// borderLeft  = UnchartedConfig.CONFIG.borderLeft.get();
		// timerMax    = 100 + UnchartedConfig.CONFIG.timer.get();
		// speed       = UnchartedConfig.CONFIG.speed.get() / 100.00f;
		// if(smallFrame){
		// 	height = 32;
		// 	timerMax -=56;
		// } else {
	...
	private void LoadBiome(){
		AbstractTexture texture = mc.getTextureManager().getTexture(new ResourceLocation(MODID , "textures/biomes/" + stringlist[listindex] + ".png"));
		if(texture instanceof DynamicTexture) { // texture file not found
			...
			biomeTexture = new ResourceLocation(MODID , "textures/biomes/" + stringlist[listindex] + ".png");
			animated = false;
		}
	}
