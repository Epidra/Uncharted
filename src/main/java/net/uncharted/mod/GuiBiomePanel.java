	...
	public void OnTravel(LivingUpdateEvent event){
		if(event.getEntity() instanceof EntityPlayer){
			if(event.getEntity().dimension == 0){
				if(event.getEntity().worldObj.canBlockSeeSky(event.getEntity().getPosition())){
					if(Biome.getIdForBiome(biomePlayer) != Biome.getIdForBiome(event.getEntity().worldObj.getBiomeGenForCoords(event.getEntity().getPosition()))){
						biomePlayer = event.getEntity().worldObj.getBiomeGenForCoords(event.getEntity().getPosition());
					}
				}
			} else {
				if(Biome.getIdForBiome(biomePlayer) != Biome.getIdForBiome(event.getEntity().worldObj.getBiomeGenForCoords(event.getEntity().getPosition()))){
